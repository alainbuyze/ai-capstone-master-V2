...
def get_preprocessor():
    ...
    numeric_features = ['previous_7', 'previous_14', 'previous_28', 'previous_70',
...
def _model_train(df, tag, test=False):
    ...
    if test:
        ...
        data_file = os.path.join(MODEL_DIR,
                                 'latest-train-{}-{}.pickle'.format(tag, model_name))
    ...
    ## update log
    print("... update_train_log\n    =>", tag, (str(dates[0]), str(dates[-1])), {'rmse': eval_rmse}, runtime,
...
def model_train(data_dir, test=False):
    ...
    for country, df in ts_data.items():
        ...
        if test and country not in ['all', 'united_kingdom']:
            continue
        print('*** Start training for Country: {}'.format(country))
...
def xmodel_predict(query, all_models=None, test=False):
    """
    function to predict from model from a dataframe of values
    """
    df = pd.DataFrame(query, columns=['countries', 'dates'])
    ...
    myresultdict = {'y_pred': [], 'y_prob': []}  # capture all the results
    # print(myresultdict)
    ### print(df)
    for index, row in df.iterrows():
        print(index, row['countries'], row['dates'], row['year'], row['month'], row['day'])
        result = model_predict(row['countries'], row['year'], row['month'], row['day'])
        print('>>> result:', result)
        myresultdict['y_pred'] += result['y_pred'].tolist()
        myresultdict['y_prob'] += result['y_proba']
...
if __name__ == "__main__":
    ...
    ## multi-entry forecast
    values = {'countries': ['all', 'france', 'netherlands', 'united_kingdom'],
              'dates': ['01-May-2018', '02-May-2018', '01-May-2018', '01-May-2018']}

    myresultdict = xmodel_predict(values)
    print(myresultdict)
