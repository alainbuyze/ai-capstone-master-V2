...
def fetch_ts(data_dir, clean=False, country=None):
    ...
    if len(os.listdir(ts_data_dir)) > 0:
        ...
        return (
            {re.sub("\.csv", "", cf)[3:]: pd.read_csv(os.path.join(ts_data_dir, cf)) for cf in os.listdir(ts_data_dir)})
    ...
    if country == None: ## default action to fetch all countries
        dfs = {'all': convert_to_ts(df)}
    ...
    elif country == "all": # only fetch the 'all' data
        dfs = {'all': convert_to_ts(df)}
    else: # fetch a specific country
        country_id = re.sub("\s+", "_", country.lower())
        dfs[country_id] = convert_to_ts(df, country=country)
...
if __name__ == "__main__":
    print('calling common functions module')
